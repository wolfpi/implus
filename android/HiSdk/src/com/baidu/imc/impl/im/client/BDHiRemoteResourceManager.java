 ...
 * <b>远程资源管理器</b>
 * <p>
 * 用于远程资源的下载，用户可提供自己的远程资源管理器负责保存用户发送的文件， 或使用SDK默认的，需要在管理后台进行配置进行开启，否则所有文件传输消息均会失败
 * </p>
 *
 * @since 1.0
 * @author WuBin
...
public class BDHiRemoteResourceManager implements RemoteResourceManager {
    ...
    public BDHiRemoteResourceManager(LocalResourceManager localResourceManager, IMConfig imConfig) {
        this.imConfig = imConfig;
        this.localResourceManager = localResourceManager;
        this.transactionFlow = new IMFileTransactionFlow();
    }

    public void uploadFile(BDHiFile file, IMTransactionFileUploadCallback callback) {
        transactionFlow.uploadFile(localResourceManager, imConfig.bosHost, file, callback);
    }

    public void downloadFile(BDHiFile file, IMTransactionFileDownloadCallback callback) {
        transactionFlow.downloadFile(localResourceManager, imConfig.bosHost, file, callback);
    }
     ...
     * <b>上传文件</b>
     * <p>
     * 上传成功后，progressListener的onSuccess方法应返回文件下载路径downloadURL， 开发者实现时，应将错误归结到以下两类错误码中，以便SDK完成附件上传和重试逻辑
     * </p>
     * <b>错误码</b>
     * <ul>
     ...
     * <b>下载文件</b>
     * <p>
     * 下载成功后，progressListener的onSuccess方法应返回文件的本地资源管理器路径filePath， 开发者实现时，应将错误归结到以下两类错误码中，以避免对无效资源的多次重复下载申请
     * </p>
     * <b>错误码</b>
     * <ul>
